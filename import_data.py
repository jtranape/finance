...
def profile(Ticker):
	...
	try:
		df=pd.read_pickle("./data/profile_"+Ticker+".pkl")											#we don't want to scrap yahoo if we already saved the data localy	
	except:	
		print("Data doesn't exist, importing...")
		...
		for i in range(1,len(dfread)):																#merging datafrane
			df=df.append(dfread[i])
		df.to_pickle("./data/profile_"+Ticker+".pkl")												#save scrapped data
		time.sleep(random.random())
...
def main():
	...
	r= requests.get(URLFinancials)
	data=r.text
	soup=bs.BeautifulSoup(data,'lxml')#html.parser')
	#print(soup)
	#container=soup.div.findAll("div")#div",{"id":"Main"})->this works
	#container=soup.div.findAll("div",{"id":"Main"})
	container=soup.div.findAll("div")
	#print(len(container))
	#for table_data in container[0].div.table
	#print(container[0].div)
	table=container[0].find_all('span')
	#table_rows = container[0].find_all('span')->this works
	#print("span " + str(len(table)))
	#print(table)
	table_begin=False
	dftable=pd.DataFrame(index=[0], columns=['Breakdown'])#create empty dataframe
	for span in table:
		#search for breakdown the first cell of the table->help
		print(span.text)
		if span.text =="":
			break
		if span.text == "Breakdown":
	
			table_begin=True 
		if table_begin:
			#print (span.text)
			if span.text.count('/')==2 : #check if it is a date
				dftable.insert(len(dftable.columns),datetime.datetime.strptime(span.text, '%m/%d/%Y'),0)
			if span.text=="ttm": #check if it is a date
				dftable.insert(len(dftable.columns),span.text,'NaN')
			if not(span.text.count('/')==2 or span.text=="tmm") and span.text[-1].isalpha():#add new ligne and add description to first column
				dftable=dftable.append([{'Breakdown': span.text}],ignore_index=True)
				i=0
			elif span.text.count(',')!=0 and not span.text[-1].isalpha(): #Then it should be a number because the last character is a number
				#print("last valid index: "+ str(dftable.last_valid_index()))
				i=i+1
				dftable.iloc[-1,i]=float(span.text.replace(',','')) #populating the first non valid dataset and convert it to float
			if span.text=="-":#limitation: this seems not to work if there is no span meaning that if there is one data missing, this shift all the value in the ligne
				i=i+1
	...
	#print(table)
	table_begin=False
	dfcf=pd.DataFrame(index=[0], columns=['Breakdown'])#create empty dataframe
	for span in table:
		#search for breakdown the first cell of the table->help
		print(span.text)
		if span.text =="":
		...
		if table_begin:
			#print (span.text)
			if span.text.count('/')==2 : #check if it is a date
				dfcf.insert(len(dfcf.columns),datetime.datetime.strptime(span.text, '%m/%d/%Y'),0)
			if span.text=="ttm": #check if it is a date
				dfcf.insert(len(dfcf.columns),span.text,'NaN')
			if not(span.text.count('/')==2 or span.text=="tmm") and span.text[-1].isalpha():#add new ligne and add description to first column
				dfcf=dfcf.append([{'Breakdown': span.text}],ignore_index=True)
				i=0
			elif span.text.count(',')!=0 and not span.text[-1].isalpha(): #Then it should be a number because the last character is a number
				#print("last valid index: "+ str(dfcf.last_valid_index()))
				i=i+1
				dfcf.iloc[-1,i]=float(span.text.replace(',','')) #populating the first non valid dataset and convert it to float
			if span.text=="-":#limitation: this seems not to work if there is no span meaning that if there is one data missing, this shift all the value in the ligne
				i=i+1
...
'''
	...
	#print(table)
	table_begin=False
	dfstatement=pd.DataFrame(index=[0], columns=['Breakdown'])#create empty dataframe
	for span in table:
		#search for breakdown the first cell of the table->help
		print(span.text)
		if span.text =="":
		...
		if table_begin:
			#print (span.text)
			if span.text.count('/')==2 : #check if it is a date
				dfstatement.insert(len(dfstatement.columns),datetime.datetime.strptime(span.text, '%m/%d/%Y'),0)
			if span.text=="ttm": #check if it is a date
				dfstatement.insert(len(dfstatement.columns),span.text,'NaN')
			if not(span.text.count('/')==2 or span.text=="tmm") and span.text[-1].isalpha():#add new ligne and add description to first column
				dfstatement=dfstatement.append([{'Breakdown': span.text}],ignore_index=True)
				i=0
			elif span.text.count(',')!=0 and not span.text[-1].isalpha(): #Then it should be a number because the last character is a number
				#print("last valid index: "+ str(dftable.last_valid_index()))
				i=i+1
				dfstatement.iloc[-1,i]=float(span.text.replace(',','')) #populating the first non valid dataset and convert it to float
			if span.text=="-":#limitation: this seems not to work if there is no span meaning that if there is one data missing, this shift all the value in the ligne
				i=i+1'''
